...
void loop(const char* window_name, Callbacks callbacks) {
    ...
    glfwSetKeyCallback(window, [](GLFWwindow* window, int key, int scancode, int action, int mods) { //NOLINT(bugprone-easily-swappable-parameters)
        CallbackUserDataFromWindow(window).key_callback(key, scancode, action, mods);
        ImGui_ImplGlfw_KeyCallback(window, key, scancode, action, mods);
    });

    glfwSetMouseButtonCallback(window, [](GLFWwindow* window, int button, int action, int mods) { //NOLINT(bugprone-easily-swappable-parameters)
        CallbackUserDataFromWindow(window).mouse_button_callback(button, action, mods);
        ImGui_ImplGlfw_MouseButtonCallback(window, button, action, mods);
    });

    glfwSetCursorPosCallback(window, [](GLFWwindow* window, double xpos, double ypos) { //NOLINT(bugprone-easily-swappable-parameters)
        CallbackUserDataFromWindow(window).cursor_position_callback(xpos, ypos);
        ImGui_ImplGlfw_CursorPosCallback(window, xpos, ypos);
    });


    glfwSetScrollCallback(window, [](GLFWwindow* window, double xoffset, double yoffset) { //NOLINT(bugprone-easily-swappable-parameters)
        CallbackUserDataFromWindow(window).scroll_callback(xoffset, yoffset);
        ImGui_ImplGlfw_ScrollCallback(window, xoffset, yoffset);
    });

    glfwSetWindowSizeCallback(window, [](GLFWwindow* window, int width, int height) { //NOLINT(bugprone-easily-swappable-parameters)
